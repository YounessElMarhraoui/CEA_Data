...
class CNNModel(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.conv1 = tf.keras.layers.Conv2D(64, kernel_size=2, activation=tf.nn.relu)
        self.conv2 = tf.keras.layers.Conv2D(32, kernel_size=2, activation=tf.nn.relu)
...
class CNNModel_comp(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.conv1 = tf.keras.layers.Conv2D(filters=64, kernel_size=2, padding="same")
        ...
        self.conv3 = tf.keras.layers.Conv2D(filters=64, kernel_size=2, padding="same")
...
class MultiCNNModel(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.cnn1_acc_x = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')
        self.cnn1_acc_y = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')
        self.cnn1_acc_z = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')

        self.cnn1_gyro_x = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')
        self.cnn1_gyro_y = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')
        self.cnn1_gyro_z = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')

        self.cnn2_acc_x = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        self.cnn2_acc_y = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        self.cnn2_acc_z = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')

        self.cnn2_gyro_x = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        self.cnn2_gyro_y = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        self.cnn2_gyro_z = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        ...
        self.cnn3_acc_x = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')
        self.cnn3_acc_y = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')
        self.cnn3_acc_z = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')

        self.cnn3_gyro_x = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')
        self.cnn3_gyro_y = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')
        self.cnn3_gyro_z = tf.keras.layers.Conv1D(filters=8, kernel_size=2, padding="same", activation='selu')

        self.cnn4_acc_x = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        self.cnn4_acc_y = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        self.cnn4_acc_z = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')

        self.cnn4_gyro_x = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        self.cnn4_gyro_y = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
        self.cnn4_gyro_z = tf.keras.layers.Conv1D(filters=16, kernel_size=2, padding="same", activation='selu')
...
class ConvLSTMConvTranspose(tf.keras.Model):
    """
    A class to create the arcitecture of the DNN model
    ...
    def __init__(self):
        ...
        Initialize the layers of the model
        """
        super(ConvLSTMConvTranspose, self).__init__()

        self.conv1 = tf.keras.layers.ConvLSTM2D(filters=16, kernel_size=2, padding="same", return_sequences=True, activation='relu')
        self.bn1 = tf.keras.layers.BatchNormalization()
        self.act1 = tf.keras.layers.ReLU()

        self.conv2 = tf.keras.layers.ConvLSTM2D(filters=16, kernel_size=2, padding="same", activation='relu')
        self.bn2 = tf.keras.layers.BatchNormalization()
        self.act2 = tf.keras.layers.ReLU()

        self.conv2_ = tf.keras.layers.Conv2DTranspose(filters=16, kernel_size=2, padding="same", activation='relu')
        self.bn2_ = tf.keras.layers.BatchNormalization()
        self.act2_ = tf.keras.layers.ReLU()

        self.conv3 = tf.keras.layers.Conv2DTranspose(filters=16, kernel_size=2, padding="same", activation='relu')
        self.bn3 = tf.keras.layers.BatchNormalization()
        self.act3 = tf.keras.layers.ReLU()
    ...
    def call(self, inputs):
        """Forward propagates the inputs into the model
